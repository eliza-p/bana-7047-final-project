...
```
```{r weeknum_check include=FALSE}
# make sure WEEK_NUM level reset
summary(organic_week$WEEK_NUM)
```

```{r agr_week,include=FALSE}
...
# Organic Weekly Agregate TRAN_NUM
ggplot(organic_SPEND_week, aes(WEEK_NUM)) + 
  geom_line(aes(y = TRAN_NUM_16, colour = "TRAN_NUM_16")) + 
  geom_line(aes(y = TRAN_NUM_17, colour = "TRAN_NUM_17"))+
  theme(legend.position="bottom")+
  ggtitle("Organic Weekly Agregate TRAN_NUM")

# Organic Weekly Agregate UNI_NUM
ggplot(organic_SPEND_week, aes(WEEK_NUM)) + 
  geom_line(aes(y = UNI_NUM_16, colour = "UNI_NUM_16")) + 
  geom_line(aes(y = UNI_NUM_17, colour = "UNI_NUM_17"))+
  theme(legend.position="bottom")+
  ggtitle("Organic Weekly Agregate UNI_NUM")

```
```{r re-merge-org,include=FALSE}
# re-merge organic data by reset WEEK_NUM
organic_WEEK <- rbind(organic_2016,organic_2017)
organic_WEEK <- organic_WEEK%>%arrange(DATE)%>%
  mutate(HSHD_NUM
...
summary(organic_WEEK)
```
```{r rf_org, include=FALSE}
# random forest
library(randomForest)
org_spend_rf<- randomForest(SPEND~HSHD_NUM+PRODUCT_NUM+REGION+WEEK_NUM+COMMODITY+BRAND_TY+BRAND_TY, data = organic_WEEK, importance=TRUE)
org_spend_rf
```



```{r get_org_name,include=FALSE}
# check table name
colnames(organic)
```
```{r spend_count,include=FALSE}
# aggregate data by "DATE" and do numeric vaible count
...
```
```{r reg_ltow,include=FALSE}
# reshape REGION count long to wide
library(data.table)
org_region <- organic[c("DATE" ,"REGION")]
org_region <- dcast(melt(setDT(org_region), id.var=c("DATE"))[value!=""], DATE ~ value, value.var="DATE", length)
head(org_region)
```
```{r dep_ltow,include=FALSE}
# reshape DEPARTMENT count long to wide
org_dep <- organic[c("DATE" ,"DEPARTMENT")]
org_dep <- dcast(melt(setDT(org_dep), id.var=c("DATE"))[value!=""], DATE ~ value, value.var="DATE", length)
head(org_dep)
```
```{r com_ltow,include=FALSE}
# reshape COMMODITY count long to wide
org_com <- organic[c("DATE" ,"COMMODITY")]
org_com <- dcast(melt(setDT(org_com), id.var=c("DATE"))[value!=""], DATE ~ value, value.var="DATE", length)
head(org_com)
```
```{r bra_ltow,include=FALSE}
# reshape BRAND_TY count long to wide
org_bra <- organic[c("DATE" ,"BRAND_TY")]
org_bra <- dcast(melt(setDT(org_bra), id.var=c("DATE"))[value!=""], DATE ~ value, value.var="DATE", length)
head(org_bra)
```
```{r loy_ltow,include=FALSE }
# reshape L count long to wide
org_loy <- organic[c("DATE" ,"L")]
org_loy <- dcast(melt(setDT(org_loy), id.var=c("DATE"))[value!=""], DATE ~ value, value.var="DATE", length)
head(org_loy)
```
```{r merge_daily_n ,include=FALSE}
organic_DATE <- merge(organic_SPEND_DATE,org_region,by="DATE")
organic_DATE <- merge(organic_DATE,org_dep,by="DATE")
organic_DATE <- merge(organic_DATE,org_com,by="DATE")
organic_DATE <- merge(organic_DATE,org_bra,by="DATE")
organic_DATE <- merge(organic_DATE,org_loy,by="DATE")
head(organic_DATE)
```
```{r lm_keys,include=FALSE}
organic_DATE_s <- organic_DATE[,c(2:37)]
summary(lm(SPEND_SUM~SPEND_AVG+TRAN_NUM+UNI_NUM,organic_DATE_s))
```
```{r}
#colnames(organic_DATE_s)
```

```{r gam_key,include=FALSE}
library(mgcv)
summary(gam(SPEND_SUM~s(SPEND_AVG)+s(TRAN_NUM)+s(UNI_NUM),data=organic_DATE_s))
```
