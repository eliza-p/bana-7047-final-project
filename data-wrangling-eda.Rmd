...
str(merge_df)
```
```{r reset merge_df , include=FALSE}
# set HSHD_NUM/PRODUCT_NUM /BASKET_NUM/YEAR to interger variable
merge_df[sapply(merge_df, is.character)] <- lapply(merge_df[sapply(merge_df, is.character)],as.integer)
merge_df$YEAR <- as.integer(merge_df$YEAR)
str(merge_df)
```


```{r split organic data, include=FALSE}
